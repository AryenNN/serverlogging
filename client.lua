    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkArray (Melee, death) then
                TriggerServerEvent('playerDied', killername .. " sopa ile öldürüldü " .. playerName)
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('playerDied', killername .. " vuruldu " .. playerName)
            elseif checkArray (Knife, death) then
                TriggerServerEvent('playerDied', killername .. " bıçaklandı " .. playerName)
            elseif checkArray (Car, death) then
                TriggerServerEvent('playerDied', killername .. " döverek öldürüldü " .. playerName)
            elseif checkArray (Animal, death) then
                TriggerServerEvent('playerDied', playerName .. " bir hayvan tarafından öldü")
            elseif checkArray (FallDamage, death) then
                TriggerServerEvent('playerDied', playerName .. " düşme hasarından öldü")
            elseif checkArray (Explosion, death) then
                TriggerServerEvent('playerDied', playerName .. " bir patlamadan öldü")
            elseif checkArray (Gas, death) then
                TriggerServerEvent('playerDied', playerName .. " Gazdan öldü")
            elseif checkArray (Burn, death) then
                TriggerServerEvent('playerDied', playerName .. " Yanarak öldü")
            elseif checkArray (Drown, death) then
                TriggerServerEvent('playerDied', playerName .. " boğuldu")
            else
                TriggerServerEvent('playerDied', playerName .. " bilinmeyen bir güç tarafından öldürüldü")
                --print(death)
            end
